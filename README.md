...
2. ZIPを任意の場所に解凍する
3. Chromeで [拡張機能画面](chrome://extensions/) を開く
...
初期設定では `Ctrl+Shift+9` が次、`Ctrl+Shift+8` が前です。FF14のキーバインドと衝突する場合は、ショートカット設定画面で別のキーに変更してください。Chromeの仕様上、環境によってはショートカット画面で手動設定が必要です。`chrome://` のリンクが開かない場合は、URLをChromeのアドレスバーに入力してください。
